...
/** Preview/demo mode is opt-in: enabled via a `?demo` query param at runtime,
 * or baked in at build time with `VITE_DEMO=true`. */
function isDemoMode(): boolean {
  return (
    import.meta.env.VITE_DEMO === 'true' || new URLSearchParams(window.location.search).has('demo')
  )
}
...
export function App() {
  ...
  useEffect(() => {
    if (!isDemoMode() || demoSeededRef.current) return
  ...
  return (
    <ThemeProvider>
      {(!settings && isLoading) || demoSeeding ? (
        <LoadingScreen />
      ) : isError ? (
