...
import { CATEGORY_KIND_BY_ID } from '@/constants/transactionCategories'
...
class MizanDB extends Dexie {
  ...
  constructor() {
    ...
    // Income/Expense category taxonomy. Adds `Category.kind` (unindexed) and
    // the new income + subcategory rows. `kind` backfills from the taxonomy's
    // static id→kind map (legacy rows only ever contained the original seed
    // ids), and any taxonomy rows missing from an existing install are added
    // so pre-existing users get the full income/expense lists.
    this.version(10)
      .stores(CURRENT_SCHEMA)
      .upgrade(async (tx) => {
        const categories = tx.table('categories')
        await categories.toCollection().modify((c: Category) => {
          if (!c.kind) c.kind = CATEGORY_KIND_BY_ID[c.id] ?? 'expense'
        })
        for (const cat of DEFAULT_CATEGORIES) {
          if (!(await categories.get(cat.id))) await categories.add(cat)
        }
      })
  ...
  private async seed() {
    if (pendingLegacySnapshot) {
      ...
      // Legacy rows predate the taxonomy split — backfill `kind` and add any
      // missing income/subcategory rows so renamed installs get the full lists.
      await this.ensureCategoryTaxonomy()
      return
    }
  ...
  /** Idempotent backfill of `Category.kind` plus any missing taxonomy rows.
   * Used by the legacy-rename path (whose snapshot predates `kind`); the v10
   * upgrade handles the equivalent case for the normal upgrade path. */
  private async ensureCategoryTaxonomy(): Promise<void> {
    await this.categories.toCollection().modify((c: Category) => {
      if (!c.kind) c.kind = CATEGORY_KIND_BY_ID[c.id] ?? 'expense'
    })
    for (const cat of DEFAULT_CATEGORIES) {
      if (!(await this.categories.get(cat.id))) await this.categories.add(cat)
    }
  }
