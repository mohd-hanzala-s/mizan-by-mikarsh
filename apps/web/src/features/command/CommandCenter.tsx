...
type CommandResult =
  ...
  | { type: 'feature_off'; feature: string; query: string }
...
const SUGGESTIONS: { text: string; requires?: keyof AiFeatures }[] = [
  { text: 'Show my biggest expenses' },
  { text: "What's my spending this month?" },
  { text: 'How much should I save monthly?', requires: 'budgetSuggestions' },
  { text: 'How much debt do I have?' },
  { text: 'How am I doing on budgets?' },
  { text: 'Which goal is closest?', requires: 'goalPrediction' },
  { text: 'Find duplicate subscriptions' },
  { text: 'Predict my net worth in 5 years', requires: 'forecasting' },
  { text: 'Can I afford a 50000 vacation?' },
  { text: 'Am I on track for retirement?', requires: 'forecasting' },
]
...
function categorySums(txs: Transaction[], categories: Category[]): Map<string, number> {
  ...
  for (const t of filterExpense(txs)) {
    const name = catById.get(rollupCategoryId(t.categoryId))?.name ?? 'Uncategorized'
...
function executeCommand(
  ...
  if (q.includes('predict') && q.includes('net worth')) {
    if (!features.forecasting)
      return { type: 'feature_off', feature: 'Forecasting', query: q.trim() }
  ...
  if (
    ...
    if (!features.budgetSuggestions)
      return { type: 'feature_off', feature: 'Budget Suggestions', query: q.trim() }
    const currentMonth = monthTransactions(transactions, 0, reference)
  ...
  if (q.includes('closest goal') || q.includes('which goal') || q.includes('nearest goal')) {
    if (!features.goalPrediction)
      return { type: 'feature_off', feature: 'Goal Prediction', query: q.trim() }
...
function resultToCard(result: CommandResult): ResultCard | null {
  switch (result.type) {
    ...
    case 'feature_off':
      return {
        icon: Zap,
        title: `${result.feature} is turned off`,
        subtitle: `This answer needs "${result.feature}", which is disabled. Enable it to get this insight.`,
        action: { label: 'Open settings', path: '/profile' },
        color: '#A4B2AD',
      }
    default:
      return null
...
export function CommandCenter() {
  ...
  const suggestions = SUGGESTIONS.filter((s) => !s.requires || aiFeatures[s.requires])
