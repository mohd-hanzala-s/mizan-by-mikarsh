...
import { presetForModeChange } from '@/services/ThemeService'
...
const RISK_OPTIONS: { value: RiskAppetite; label: string; desc: string; icon: typeof Shield }[] = [
  {
    value: 'conservative',
    label: 'Conservative',
    desc: 'Prefer stable, low-risk financial choices. Focus on safety and capital preservation.',
    icon: Shield,
  },
  {
    value: 'balanced',
    label: 'Balanced',
    desc: 'Comfortable with moderate risk for moderate returns. Mix of safe and growth assets.',
    icon: TrendingUp,
  },
  {
    value: 'aggressive',
    label: 'Aggressive',
    desc: 'Willing to accept higher risk for potentially higher returns. Growth-oriented strategy.',
    icon: TrendingUp,
  },
]
...
function SectionHeader({
  ...
  return (
    ...
    >
      ...
      <ChevronDown
        className={cn(
          'size-20 text-text-tertiary transition-transform duration-fast',
          isOpen && 'rotate-180'
        )}
        aria-hidden="true"
      />
...
export function ProfilePage() {
  ...
  const investmentsTotal = activeInvestments.reduce(
    (s, i) => s + i.units * i.currentPricePerUnit,
    0
  )
  ...
  return (
    <div className="flex flex-col gap-16 p-16 pb-32 md:p-24">
      ...
      <section className="rounded-md border border-border bg-surface-card p-16">
        <div className="flex items-center gap-16">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-body-sm capitalize text-text-tertiary">{identity.profileType}</p>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-md border border-border bg-surface-card overflow-hidden">
        ...
        {openSections.has('personal') && (
          <div className="border-t border-border-subtle px-16 py-16 space-y-16">
            <div>
              <label className="mb-4 block text-caption uppercase tracking-wide text-text-tertiary">
                Display Name
              </label>
              <input
                type="text"
            ...
            <div>
              ...
              <PillSelect
                ...
                onChange={(v) => savePartial({ profileType: v })}
              />
            </div>
      ...
      <section className="rounded-md border border-border bg-surface-card overflow-hidden">
        ...
        {openSections.has('income') && (
          <div className="border-t border-border-subtle px-16 py-16 space-y-16">
            <div>
              <label className="mb-4 block text-caption uppercase tracking-wide text-text-tertiary">
                Income ({identity.incomeFrequency})
              </label>
              <div className="relative">
                <span className="absolute left-12 top-1/2 -translate-y-1/2 text-body text-text-tertiary">
                  ₹
                </span>
                <input
                  type="number"
            ...
            <div>
              ...
              <PillSelect
                ...
                value={identity.incomeFrequency}
                onChange={(v) => savePartial({ incomeFrequency: v })}
              />
            </div>
            <div>
              <label className="mb-4 block text-caption uppercase tracking-wide text-text-tertiary">
      ...
      <section className="rounded-md border border-border bg-surface-card overflow-hidden">
        <SectionHeader
          icon={UsersIcon}
          title="Dependents & Tax"
          isOpen={openSections.has('dependents')}
          ...
          <div className="border-t border-border-subtle px-16 py-16 space-y-16">
            <div>
              ...
              <div className="flex items-center gap-12">
                ...
                <span className="min-w-32 text-center text-body-lg font-semibold tabular-nums text-text-primary">
                  {identity.dependents}
                </span>
                <button
                  type="button"
                  ...
                  >
                    <div className="flex items-center gap-10">
                      <opt.icon
                        className={cn(
                          'size-20',
                          isActive ? 'text-brand-teal900' : 'text-text-secondary'
                        )}
                        aria-hidden="true"
                      />
                      <span
                        className={cn(
                          ...
                          isActive ? 'text-brand-teal900' : 'text-text-primary'
                        )}
                      >
                        {opt.label}
                      </span>
        ...
        {openSections.has('summary') && (
          <div className="border-t border-border-subtle px-16 py-16">
            <div className="divide-y divide-border-subtle">
              <StatRow label="Total Balance" value={formatAmount(totalBalance, 'INR')} />
              <StatRow
                label="Active Goals"
                value={`${activeGoals.length} · ${formatAmount(goalsTargetTotal, 'INR')} target`}
              />
              <StatRow
                label="Active Loans"
                value={`${activeLoans.length} · ${formatAmount(loansBalanceTotal, 'INR')} balance`}
              />
              <StatRow label="Active Budgets" value={String(activeBudgets.length)} />
              <StatRow label="Total Investments" value={formatAmount(investmentsTotal, 'INR')} />
          ...
          <div className="border-t border-border-subtle px-16 py-16 space-y-12">
                ...
                {THEME_OPTIONS.map(({ value, label, icon: Icon }) => (
                  <button
                    ...
                    onClick={() =>
                      updateSettings({
                        theme: value,
                        themePreset: presetForModeChange(value, settings.themePreset),
                      })
                    }
                    className={cn(
            ...
            <div className="flex items-center justify-between gap-12">
              ...
              <select
                ...
                onChange={(e) =>
                  updateSettings({
                    currencyDisplay: e.target.value as 'lakh-crore' | 'international',
                  })
                }
            ...
            <div className="flex items-center justify-between gap-12">
              ...
              <select
                ...
                onChange={(e) =>
                  updateSettings({
                    backupFrequency: e.target.value as 'off' | 'weekly' | 'monthly',
                  })
                }
                className="rounded-md border border-border bg-surface px-8 py-4 text-body-sm text-text-primary"
        ...
        {openSections.has('ai') && (
          <div className="border-t border-border-subtle px-16 py-16 space-y-6">
            <Toggle
              ...
              onChange={(v) =>
                savePartial({ aiFeatures: { ...identity.aiFeatures, forecasting: v } })
              }
              label="Forecasting"
            ...
            <Toggle
              ...
              onChange={(v) =>
                savePartial({ aiFeatures: { ...identity.aiFeatures, recommendations: v } })
              }
            ...
            <Toggle
              ...
              onChange={(v) =>
                savePartial({ aiFeatures: { ...identity.aiFeatures, autoCategorize: v } })
              }
            ...
            <Toggle
              ...
              onChange={(v) =>
                savePartial({ aiFeatures: { ...identity.aiFeatures, budgetSuggestions: v } })
              }
            ...
            <Toggle
              ...
              onChange={(v) =>
                savePartial({ aiFeatures: { ...identity.aiFeatures, goalPrediction: v } })
              }
      ...
      {settings.developerMode && (
        <section className="rounded-md border border-border bg-surface-card overflow-hidden">
          <SectionHeader
            icon={Code}
            title="Developer"
            isOpen={openSections.has('developer')}
          ...
          {openSections.has('developer') && (
            <div className="border-t border-border-subtle px-16 py-16 space-y-12">
              ...
              <div className="rounded-md border border-border bg-surface p-12">
                <p className="mb-8 text-caption uppercase tracking-wide text-text-tertiary">
                  API Tokens
                </p>
                <p className="text-body-sm text-text-tertiary">
                  API token management is not yet available.
                </p>
              </div>
            </div>
          )}
      ...
      <section className="rounded-md border border-expense/30 bg-surface-card overflow-hidden">
        ...
        >
          ...
          <ChevronDown
            className={cn(
              'size-20 text-text-tertiary transition-transform duration-fast',
              openSections.has('danger') && 'rotate-180'
            )}
            aria-hidden="true"
          />
        </button>
        {openSections.has('danger') && (
          <div className="border-t border-expense/20 px-16 py-16">
            <p className="mb-12 text-body-sm text-text-secondary">
              This will reset all your financial identity preferences to their defaults. This action
              cannot be undone.
            </p>
            {!resetConfirm ? (
              ...
              <div className="space-y-8">
                <p className="text-body-sm font-semibold text-expense">
