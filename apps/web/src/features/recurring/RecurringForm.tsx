...
export function RecurringForm({ editing, onSaved, onCancel }: RecurringFormProps) {
  ...
  // Reset the category when the rule's type changes so an expense category
  // can never be paired with an income rule (skips the initial render so
  // edit mode keeps its pre-filled category).
  ...
  // If the pre-selected account was archived (e.g. the seeded `acc-cash`),
  // fall back to the first active account so a rule never targets a hidden
  // one. React's documented "adjust state during render" pattern.
  ...
  return (
    <div className="flex flex-col gap-24">
      ...
      <div className="flex flex-col gap-8">
        ...
        <CategorySelector value={categoryId} onChange={setCategoryId} type={type} />
      </div>
        ...
        {frequency === 'custom' && (
          <div className="flex items-center gap-12">
            ...
            <input
              ...
              onChange={(e) =>
                setCustomIntervalDays(e.target.value ? Number(e.target.value) : null)
              }
              aria-label="Custom interval in days"
              className="w-64 rounded-md border border-border bg-surface-card px-12 py-8 text-body text-text-primary outline-none focus:border-income"
            />
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="flex items-center gap-12">
          <input
            ...
            max={7}
            value={reminderDays}
            onChange={(e) => setReminderDays(Math.min(7, Math.max(0, Number(e.target.value) || 0)))}
