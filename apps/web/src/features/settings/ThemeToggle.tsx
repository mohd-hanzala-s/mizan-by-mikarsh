...
const presetSwatches: Record<ThemePreset, string> = {
  ...
  modern: 'linear-gradient(135deg, #F6F8FB 50%, #2563EB 50%)',
  paper: 'linear-gradient(135deg, #FAF7F0 50%, #302A20 50%)',
  soft: 'linear-gradient(135deg, #F6F2FB 50%, #7C5FB8 50%)',
  midnight: 'linear-gradient(135deg, #0A1613 50%, #183F2D 50%)',
  oled: 'linear-gradient(135deg, #000000 50%, #181818 50%)',
  terminal: 'linear-gradient(135deg, #020C04 50%, #00FF00 50%)',
  violet: 'linear-gradient(135deg, #120C20 50%, #5B3FA8 50%)',
}
...
export function ThemeToggle() {
  ...
  const handleModeChange = (value: ThemePreference) => {
    update({ theme: value, themePreset: presetForModeChange(value, settings?.themePreset) })
  }
...
export function AppearanceSettings() {
  ...
  const themes = getThemesForMode(mode)
      ...
      <section className="flex flex-col gap-12">
        ...
        <div className="card-sm p-20 flex flex-col gap-16">
          ...
          <p className="text-body-sm text-text-tertiary">
            {mode === 'dark'
              ? 'Choose a dark variant. Switch to Light mode to see light themes.'
              : 'Choose a light variant. Switch to Dark mode to see dark themes.'}
          </p>
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-4">
            {themes.map((presetDef) => (
              <button
                key={presetDef.id}
                type="button"
