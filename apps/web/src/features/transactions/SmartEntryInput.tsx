...
export function SmartEntryInput({ onSaved, initial, editingId }: SmartEntryInputProps) {
  ...
  // If the pre-selected account was archived (e.g. the seeded `acc-cash`), the
  // AccountSelector below can no longer display it — fall back to the first
  // active account so a new entry never silently posts to a hidden one. Uses
  // React's documented "adjust state during render" pattern (no effect round-trip).
  ...
  // Automatic reset on type switch: whenever the transaction type changes
  // (manual toggle or a parsed inference like "5000 salary"), drop the
  // selected category and suggestion so an expense category can never be
  // submitted against an income entry. Skips the initial render so edit mode
  // keeps its pre-filled category.
  ...
  useEffect(() => {
    ...
    if (!aiFeatures.autoCategorize) return
    let cancelled = false
    CategorizationService.suggest(parsed.description, type).then((result) => {
    ...
    // react to the description (or type) changing, not to the user touching category.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [parsed.description, aiFeatures.autoCategorize, type])
  ...
  return (
    <div className="flex flex-col gap-24">
        ...
        {(['expense', 'income'] as const).map((t) => (
          <button
            ...
            className={cn(
              ...
              type === t
                ? t === 'expense'
                  ? 'bg-surface-card text-expense shadow-sm'
                  : 'bg-surface-card text-income shadow-sm'
                : 'text-text-secondary'
            )}
          >
            {t}
      ...
      <div className="flex flex-col gap-8">
        <span className="text-overline text-text-tertiary">Tags</span>
