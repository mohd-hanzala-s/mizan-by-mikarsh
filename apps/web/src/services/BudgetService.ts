 ...
 * stays correct once it is). Subcategory transactions roll up into their
 * parent so a "Restaurants" entry counts against the "Food" budget. */
function sumSpent(transactions: Transaction[], categoryId: string, range: DateRange): number {
  return transactions
    .filter((t) => !t.isDeleted && inRange(t.transactionDate, range.start, range.end))
    .filter(
      (t) =>
        categoryId === GLOBAL_BUDGET_CATEGORY_ID || rollupCategoryId(t.categoryId) === categoryId
    )
    .reduce((sum, t) => {
...
export function computeBudgetStatus(
  ...
  const rollover = budget.rolloverEnabled
    ? budget.monthlyLimit -
      sumSpent(transactions, budget.categoryId, getPreviousPeriod(budgetMonthStart, reference))
    : 0
  ...
  const daysElapsed = Math.min(
    daysInPeriod,
    Math.max(1, Math.floor((reference.getTime() - period.start.getTime()) / DAY_MS) + 1)
  )
  ...
  return { budget, period, allocated, spent, remaining, percentUsed, forecastEndOfPeriod, severity }
}
...
export const BudgetService = {
   ...
   * itself via color. */
  getAlerts(statuses: BudgetStatus[], categories: Category[]): DashboardAlert[] {
  ...
  async create(input: CreateBudgetInput): Promise<Budget> {
    if (!(input.monthlyLimit > 0)) throw new Error('Monthly limit must be greater than 0.')
  ...
  async update(id: string, input: Omit<CreateBudgetInput, 'categoryId'>): Promise<void> {
    if (!(input.monthlyLimit > 0)) throw new Error('Monthly limit must be greater than 0.')
