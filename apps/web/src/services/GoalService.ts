...
export const GoalService = {
  ...
  computeProgress(goal: Goal, options?: { predictions?: boolean }): GoalProgress {
    const predictions = options?.predictions ?? true
    const percentage =
      goal.targetAmount > 0
    ...
    if (predictions && goal.deadline && goal.targetAmount > 0 && goal.status === 'active') {
