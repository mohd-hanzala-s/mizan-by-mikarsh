...
function dueDateForMonth(dueDay: number, year: number, monthIndex: number): Date {
...
export function firstDueDate(loan: Loan): Date {
  ...
  const due = dueDateForMonth(loan.dueDay, start.getFullYear(), start.getMonth())
...
export function nextDueDate(loan: Loan, reference = new Date()): Date | null {
  ...
  let cursor = new Date(firstDueDate(loan).getFullYear(), firstDueDate(loan).getMonth(), 1)
...
function latestDueDateOnOrBefore(loan: Loan, reference: Date): Date | null {
  ...
  for (;;) {
    ...
    const candidate = dueDateForMonth(loan.dueDay, next.getFullYear(), next.getMonth())
 ...
 */
export function isOverdue(loan: Loan, payments: LoanPayment[], reference = new Date()): boolean {
  ...
  const previousDue = dueDateForMonth(loan.dueDay, previous.getFullYear(), previous.getMonth())
  return !payments.some((p) => parseDay(p.paymentDate).getTime() > previousDue.getTime())
}
...
export function getPayoffForecast(loan: Loan, reference = new Date()): PayoffForecast {
  const progress =
    loan.originalAmount > 0
      ? Math.min(1, Math.max(0, (loan.originalAmount - loan.currentBalance) / loan.originalAmount))
      : 0
  ...
  if (monthlyRate <= 0) {
    const remainingEmis = Math.max(1, Math.ceil(loan.currentBalance / loan.monthlyEMI))
    return { progress, remainingEmis, completionDate: addMonths(anchor, remainingEmis - 1) }
  }
  ...
  if (balance > 0) return { progress, remainingEmis: null, completionDate: null }
  return { progress, remainingEmis: months, completionDate: addMonths(anchor, months - 1) }
}
...
export type UpdateLoanInput = Omit<CreateLoanInput, 'originalAmount' | 'startDate'>
...
function validateSchedule(
  input: Pick<CreateLoanInput, 'loanName' | 'monthlyEMI' | 'interestRate' | 'dueDay' | 'endDate'>,
  startDate: string
): void {
  if (!input.loanName.trim()) throw new Error('Loan name is required.')
  if (!(input.monthlyEMI > 0)) throw new Error('Monthly EMI must be greater than 0.')
  if (input.interestRate !== null && (input.interestRate < 0 || input.interestRate > 100)) {
    throw new Error('Interest rate must be between 0 and 100.')
  }
  if (!Number.isInteger(input.dueDay) || input.dueDay < 1 || input.dueDay > 31) {
    throw new Error('Due day must be between 1 and 31.')
  }
  if (!Number.isFinite(parseDay(startDate).getTime())) throw new Error('Start date is required.')
  if (input.endDate && parseDay(input.endDate).getTime() < parseDay(startDate).getTime()) {
...
export const LoanService = {
  ...
  async create(input: CreateLoanInput): Promise<Loan> {
    ...
    if (!(input.originalAmount > 0)) throw new Error('Original amount must be greater than 0.')
  ...
  async recordPayment(loanId: string, input: RecordPaymentInput): Promise<LoanPayment> {
    ...
    const { principalPaid, interestPaid } = splitPayment(loan, input.amountPaid)
    ...
    await db.transaction('rw', db.loans, db.loan_payments, async () => {
      ...
      await LoanRepository.update(loan.id, { currentBalance: remainingBalance, status })
    })
    return payment
  },
  ...
  async deletePayment(loanId: string, paymentId: string): Promise<void> {
    ...
    if (!payment || payment.loanId !== loanId) throw new Error('Payment not found.')
    ...
    await db.transaction('rw', db.loans, db.loan_payments, async () => {
      ...
      await LoanRepository.update(loan.id, { currentBalance: restored, status })
    })
  },
  ...
  getAlerts(
    ...
    for (const loan of loans) {
      ...
      if (latest) {
        const paidOn = parseDay(latest.paymentDate)
        if (paidOn.getTime() >= recentCutoff.getTime() && latest.amountPaid > loan.monthlyEMI) {
