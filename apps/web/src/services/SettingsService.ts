...
/** Salt used before the product rename — kept so `verifyPin` can still
 * accept PINs whose hash was stored under the old salt (the DB migration
 * copies those hashes over). New PINs always use `PIN_SALT`. */
const LEGACY_PIN_SALT = 'nexus-finance-app-lock-v1'
...
export const SettingsService = {
  ...
  async disableAppLock(): Promise<Settings> {
    return SettingsRepository.update({ appLockEnabled: false, appLockPinHash: null })
  },
  ...
  async verifyPin(pin: string, storedHash: string): Promise<boolean> {
    ...
    const legacyHash = await sha256Hex(`${LEGACY_PIN_SALT}:${pin}`)
