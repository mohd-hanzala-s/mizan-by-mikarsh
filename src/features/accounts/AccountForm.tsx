...
export function AccountForm({ existing, onSaved, onCancel }: AccountFormProps) {
  ...
  async function handleSave() {
    ...
    try {
      if (existing) {
        await AccountService.update(existing.id, { name, icon, color, currency })
      } else {
  ...
  return (
    <div className="flex flex-col gap-24">
      ...
      <div className="flex flex-col gap-8">
        <span className="text-overline text-text-tertiary">Currency</span>
        <div className="flex flex-wrap gap-8">
          {SUPPORTED_CURRENCIES.map((code) => (
            <button
              ...
              className={cn(
                'min-h-touch rounded-full border px-14 text-body-sm font-medium transition-colors duration-fast',
                currency === code
                  ? 'border-income bg-income-subtle text-income'
                  : 'border-border bg-surface-card text-text-secondary'
              )}
