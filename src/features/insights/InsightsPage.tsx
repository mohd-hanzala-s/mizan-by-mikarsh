...
export function InsightsPage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const recommendations = useMemo(
    () =>
      InsightService.getRecommendations(
        transactions,
        accounts,
        budgets,
        categories,
    ...
    [transactions, accounts, budgets, categories, loans, payments, rules, budgetMonthStart]
  )
  ...
  const savings = useMemo(
    () => InsightService.getSavingsSuggestions(transactions, categories),
    [transactions, categories]
  )
  ...
  return (
    <div className={cn('flex flex-col gap-16', !embedded && 'p-16 md:p-24')}>
      ...
      <Card title="Recommendations">
        {!hasRecommendations ? (
          <p className="text-body-sm text-text-tertiary">
            No recommendations right now. Keep logging and they'll show up here when something needs
            attention.
          </p>
        ) : (
          <>
