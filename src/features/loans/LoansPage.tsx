...
type Sheet = { kind: 'add' } | { kind: 'edit'; loan: Loan } | { kind: 'payment'; loan: Loan }
...
export function LoansPage() {
  ...
  return (
    <div className="flex flex-col gap-16 p-16 md:p-24">
      ...
      {activeLoans.length >= 2 && <DebtPayoffPlanner loans={activeLoans} />}
      ...
      >
        ...
        {sheet?.kind === 'edit' && (
          <LoanForm editing={sheet.loan} onSaved={handleSaved} onCancel={() => setSheet(null)} />
        )}
        {sheet?.kind === 'payment' && (
          <PaymentForm loan={sheet.loan} onSaved={handleSaved} onCancel={() => setSheet(null)} />
        )}
      </BottomSheet>
