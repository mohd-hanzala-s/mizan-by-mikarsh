...
import type { Transaction, TransactionStatus, TransactionType } from '@/types/entities'
...
async function applyBalanceEffect(t: Transaction): Promise<void> {
  if (t.type === 'transfer') {
    ...
    await AccountRepository.adjustBalance(t.accountId, balanceEffect(t.type, t.amount))
  }
}
...
export const TransactionService = {
  ...
  async update(id: string, input: CreateTransactionInput): Promise<Transaction> {
    ...
    if (existing.type === 'transfer') {
      throw new Error('Transfers can\u2019t be edited yet — delete and recreate instead.')
    }
  ...
  async softDelete(id: string): Promise<void> {
    ...
    await db.transaction('rw', db.transactions, db.accounts, async () => {
      ...
      if (linked && !linked.isDeleted) {
        ...
        await db.transactions.update(linked.id, { isDeleted: true, updatedAt: now })
      }
    })
  },
  ...
  async restore(id: string): Promise<void> {
    ...
    await db.transaction('rw', db.transactions, db.accounts, async () => {
      ...
      if (linked && linked.isDeleted) {
        ...
        await db.transactions.update(linked.id, { isDeleted: false, updatedAt: now })
      }
    })
  },
  ...
  async duplicate(id: string): Promise<Transaction> {
    ...
    if (!existing) throw new Error(`Transaction ${id} not found`)
    if (existing.type === 'transfer') {
      throw new Error('Transfers can\u2019t be duplicated — create a new transfer instead.')
    }
  ...
  async createScheduled(input: CreateScheduledTransactionInput): Promise<Transaction> {
    if (!(input.amount > 0)) {
      throw new Error('Amount is required and must be greater than 0.')
    }
  ...
  async updateStatus(id: string, status: Exclude<TransactionStatus, 'paid'>): Promise<void> {
    const existing = await db.transactions.get(id)
    if (!existing) throw new Error(`Transaction ${id} not found`)
  ...
  async bulkImport(
    ...
    let imported = 0
    let skipped = 0
    const defaultAccountId = 'acc-cash'
    const defaultCategoryId = 'cat-other'
    const now = new Date().toISOString()
    ...
    for (const [i, row] of rows.entries()) {
      try {
        ...
        const accountId = row.accountId || defaultAccountId
        const categoryId = row.categoryId || defaultCategoryId

        const duplicate = await DuplicateDetectionService.findPossibleDuplicate({
          amount: row.amount,
        ...
        const tagObjects = await Promise.all(
          (row.tags ?? []).map((t) => TagRepository.findOrCreate(t))
        )
